  ...
  <section id='about' className="mt-8 min-h-fit flex flex-col items-center justify-center px-4 sm:px-6 md:px-12 lg:px-24 overflow-hidden">
      ...
      <div id="professional-journey" className="mt-8 w-full mx-auto flex flex-col items-center">
        ...
        >
          ...
          />
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-center bg-gradient-to-r from-red-500 to-red-700 text-transparent bg-clip-text mb-4">Who I am</h2>

          <motion.p
            variants={paragraphVariants}
            className="leading-relaxed text-center text-sm sm:text-base md:text-lg lg:text-2xl max-w-4xl mx-auto"
          >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {skillProgress.map((skill, index) => (
                <motion.div
                  key={skill.name}
                  className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-6 shadow-md hover:shadow-lg transition-shadow duration-300"
      ...
      <div className="mt-8 sm:mt-12 w-full overflow-hidden">
        <motion.div
          className="flex items-center gap-2 sm:gap-4 md:gap-8 flex-nowrap"
          animate={{ x: ['0%', '-50%'] }}
          transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
          style={{ width: '200%' }}
        >
          {icons.concat(icons).map((src, index) => (
            <div key={index} className="flex h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 items-center justify-center shrink-0">
