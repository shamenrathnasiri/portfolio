    ...
    <section className="min-h-screen relative overflow-hidden bg-gradient-to-br from-red-950 via-black to-gray-950 px-4 sm:px-6 md:px-10 py-16">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {heroStats.map((stat) => (
            <div key={stat.label} className="relative rounded-3xl border border-red-700/40 bg-gradient-to-br from-red-900/70 via-black to-black/60 p-6 overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.6)]">
              <div className="absolute inset-0 opacity-40 blur-3xl bg-[radial-gradient(circle,rgba(255,0,0,0.4),transparent_65%)]"></div>
              <div className="relative z-10 space-y-2">
                <p className="text-xs uppercase tracking-[0.4em] text-red-300">{stat.label}</p>
                <p className="text-3xl sm:text-4xl font-black text-white">{stat.value}</p>
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">{stat.sub}</p>
              </div>
            </div>
          ))}
        </div>

        <section className="relative rounded-[32px] border border-red-700/40 bg-black/40 p-6 overflow-hidden shadow-2xl">
          <div className="absolute inset-0 opacity-30 blur-3xl bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.15),transparent_70%)]"></div>
          <div className="relative z-10 space-y-6">
            <div className="flex items-center justify-between">
              <p className="text-xs tracking-[0.5em] uppercase text-red-400">Process Glyph</p>
              <span className="text-sm text-gray-400">Figma + Framer</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {processFlow.map((step) => (
                <div
                  key={step.step}
                  className="relative rounded-2xl border border-red-900/40 bg-gradient-to-br from-black/70 via-red-950/60 to-black/40 p-5 shadow-[0_15px_30px_rgba(0,0,0,0.6)]"
                >
                  <span className="text-xs tracking-[0.4em] uppercase text-red-400">{step.step}</span>
                  <h3 className="text-lg font-black text-white mt-3">{step.title}</h3>
                  <p className="text-sm text-gray-300 mt-2 leading-relaxed">{step.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="relative rounded-[32px] border border-red-700/50 bg-gradient-to-br from-red-900/80 via-black/60 to-black/80 p-6 overflow-hidden shadow-[0_0_45px_rgba(220,38,38,0.35)]">
          ...
          <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl space-y-3">
              <p className="text-xs tracking-[0.5em] uppercase text-red-400">Creative System</p>
              <h3 className="text-3xl sm:text-4xl font-black text-white leading-tight">
                Neon grids, layered glass, and red gradients keep every surface alive.
              </h3>
              <p className="text-gray-300 text-sm sm:text-base">
                Bold typography, cinematic shadows, and fluid motion lines turn static screens into immersive concept art.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-3 w-full sm:w-auto">
              <div className="flex flex-col rounded-2xl border border-red-600/60 bg-black/40 px-4 py-3 text-white">
                <span className="text-xs uppercase text-red-400">Textures</span>
                <span className="text-2xl font-black">3D</span>
                <span className="text-xs text-gray-400">noise + glass</span>
              </div>
              <div className="flex flex-col rounded-2xl border border-red-600/60 bg-black/40 px-4 py-3 text-white">
                <span className="text-xs uppercase text-red-400">Motion</span>
                <span className="text-2xl font-black">Fluid</span>
                <span className="text-xs text-gray-400">micro interactions</span>
              </div>
              <div className="flex flex-col rounded-2xl border border-red-600/60 bg-black/40 px-4 py-3 text-white">
                <span className="text-xs uppercase text-red-400">Tone</span>
                <span className="text-2xl font-black">Bold</span>
                <span className="text-xs text-gray-400">rich reds</span>
              </div>
            </div>
          </div>
        </div>
